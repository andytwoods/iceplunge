...
class PlungeListView(LoginRequiredMixin, ListView):
    ...
    def get_context_data(self, **kwargs):
        ...
        last = PlungeLog.objects.filter(user=self.request.user).order_by("-timestamp").first()
        ...
        context["form"] = PlungeLogForm(initial=initial)
        context["intensity_choices"] = [
            (val, label, desc)
            for (val, label), desc in zip(PlungeLog.INTENSITY_CHOICES, PlungeLog.INTENSITY_DESCRIPTIONS)
        ]
        daily, weekly = _covariate_instances(self.request.user)
        context["daily_form"] = DailyCovariateForm(instance=daily)
        context["weekly_form"] = WeeklyCovariateForm(instance=weekly)
        return context
    ...
    def post(self, request):
        ...
        if form.is_valid():
            ...
            if request.headers.get("HX-Request"):
                return HttpResponse(
                    _render_plunge_row(request, plunge),
...
def _render_form_partial(request, form, daily_form=None, weekly_form=None):
    from django.template.loader import render_to_string
